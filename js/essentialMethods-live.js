...
console.log($('.main-content').css("width")); // Get some information

//TODO Together: Can you help me? I'd like to set my modal's text to have a cool color :)
$('.modal-body').css('color', 'rebeccapurple'); // Set some Css (note: two arguments - a. our property and b. our value)
...
navContents += "<button id='toggle'>Toggle</button>";
$('nav').html(navContents);

$('#toggle, #colorschemeBtn').click(function(e) {
    e.preventDefault();
    $('nav').toggleClass('bg-dark navbar-dark bg-info navbar-light');
})
...
$('h1').css({'font-size': '60px', 'color': 'antiquewhite'});
...
$('.hat-sold').each(function(index, element) { // iterate over matched elements
    if(index !== 0) {
        $(element).html('Fedora hats'); // Set html to value
    }
});
...
//     $($('ol').children()[i]).html('Fedora hats');
// }
